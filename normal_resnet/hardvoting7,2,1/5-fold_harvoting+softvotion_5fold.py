...
def plot_roc_curve(y_true, y_score, fold_num, voting_type):
    ...
    plt.title(f'ROC Curve {voting_type} voting fold {fold_num}')
    plt.legend(loc="lower right")
    plt.savefig(f'roc_{voting_type}_fold_num_{fold_num}.png')
    plt.show()
    return roc_auc


def save_roc_curve(all_fpr, all_tpr, all_roc_auc, voting_type):
    ...
    std_tpr = np.std([np.interp(mean_fpr, all_fpr[i], all_tpr[i]) for i in range(10)], axis=0)

    plt.figure(figsize=(8, 8))
    plt.plot(mean_fpr, mean_tpr, color='b', label=f'Mean ROC (AUC = {np.mean(all_roc_auc):.2f} +/- {np.std(all_roc_auc):.2f})')
    plt.fill_between(mean_fpr, mean_tpr-std_tpr, mean_tpr+std_tpr, color='grey', alpha=0.2, label='Std Dev')
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    plt.title(f'{voting_type} Voting: Mean and Std Dev of ROC Curves')
    plt.legend(loc='lower right')
    plt.savefig(f'{voting_type}_voting_mean_roc_curve.png')
    plt.close()
...
for i in range(10):
    ...
    model_path = f"/home/minkyoon/crohn/normal_resnet/stratifie_10fold_7,21/fold{fold_num}_resnet50_pi.pt"
    model_image = ResNet()
    ...
    # DataLoader
    dataset = CustomImageDataset(f"/home/minkyoon/crohn/csv/normal_resnet/10fold_staratified_7,2,1/test_fold_{fold_num}.csv", transform_valid)
    ...
    # 실제 라벨을 데이터에서 가져오기
    real_labels = {}
    data_csv = pd.read_csv(f"/home/minkyoon/crohn/csv/normal_resnet/10fold_staratified_7,2,1/test_fold_{fold_num}.csv")
    ...
    results.append({
        ...
        'sensitivity': sensitivity_soft,
        'specificity': specificity_soft,
        'f1': f1_soft
    })
    ...
    # Hard voting ROC AUC
    y_score_hard = [final_predictions[key] for key in final_predictions.keys()]
    roc_auc_hard = plot_roc_curve(y_true, y_score_hard, fold_num, 'hard')
    results[-2]['roc_auc'] = roc_auc_hard
    fpr, tpr, _ = roc_curve(y_true, y_score_hard)
    roc_auc = auc(fpr, tpr)
    all_fpr_hard.append(fpr)
    all_tpr_hard.append(tpr)
    all_roc_auc_hard.append(roc_auc)

    # Soft voting ROC AUC
    y_score_soft = [probability_sums[key][1] / sum(probability_sums[key]) for key in final_predictions_soft.keys()]
    roc_auc_soft = plot_roc_curve(y_true_soft, y_score_soft, fold_num, 'soft')
    results[-1]['roc_auc'] = roc_auc_soft
    fpr, tpr, _ = roc_curve(y_true_soft, y_score_soft)
    roc_auc = auc(fpr, tpr)
    all_fpr_soft.append(fpr)
    all_tpr_soft.append(tpr)
    all_roc_auc_soft.append(roc_auc)


df_results = pd.DataFrame(results)
df_results.to_csv('voting_results_5fold.csv', index=False)

save_roc_curve(all_fpr_hard, all_tpr_hard, all_roc_auc_hard, 'Hard')
save_roc_curve(all_fpr_soft, all_tpr_soft, all_roc_auc_soft, 'Soft')