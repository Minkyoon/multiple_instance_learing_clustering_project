...
for train_val_index, test_index in skf.split(df['slide_id'], df['label']):
    ...
    sss = StratifiedShuffleSplit(n_splits=1, test_size=0.125, random_state=42)  # 0.125 = 1/(1+7) = 1/8
    ...
    split_df.to_csv(f'/home/minkyoon/crohn/csv/clam/remission/clam_startified5fold/splits_{idx}.csv', index=False, )
    idx += 1
