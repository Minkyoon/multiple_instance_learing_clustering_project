...
for train_val_index, test_index in skf.split(df['slide_id'], df['label']):
    ...
    sss = StratifiedShuffleSplit(n_splits=1, test_size=0.111, random_state=42)  # 8:1의 비율에서 validation이 차지하는 비율은 0.111 (1/9)
    ...
    split_df.to_csv(f'/home/minkyoon/first/CLAM/splits/remission_stratified_8,1,1 /splits_{idx}.csv', index=False)  # 경로를 구분하기 위해 _811 추가
    idx += 1
