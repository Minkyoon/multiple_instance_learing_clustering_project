...
df2 = pd.read_csv('/home/minkyoon/crohn/csv/clam/remission/remission_original/rem.csv')
...
merged_df = merged_df.drop(columns=['Unnamed: 0_y','Unnamed: 0_x'])
merged_df.to_csv('rem.csv', index=False)
...
# 파일 불러오기
df = pd.read_csv('/home/minkyoon/crohn/pyfile/various_pyfile/rem.csv')
...
# diff_days가 7일 이하인 행만 남기기
df_7 = df[df['diff_days'] <= 7]
print(df_7)

# diff_days가 60일 이하인 행의 수 확인
count_7 = df[df['diff_days'] <= 7].shape[0]
print(f"Number of rows where difference is less than or equal to 7 days: {count_7}")
df_7 = df_7[['serial_number', 'remission_under30']]
df_7.to_csv('real_remission.csv', index=False)
...
df2 = pd.read_csv('/home/minkyoon/crohn/pyfile/various_pyfile/real_remission.csv')
...
merged_df['label'] = merged_df['remission_under30']
