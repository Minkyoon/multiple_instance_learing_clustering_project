...
batch=args.batch
# 데이터 로더 생성
train_dataset = CustomDataset(csv_file="/home/minkyoon/crohn/csv/label_data/crohn/train.csv")
val_dataset= CustomDataset(csv_file="/home/minkyoon/crohn/csv/label_data/crohn/valid.csv")
test_dataset=CustomDataset(csv_file="/home/minkyoon/crohn/csv/label_data/crohn/test.csv")
...
torch.save(model_best.state_dict(), 'crohn3.pt')
...
save_path = 'loss_curve3.png'
plot_loss_curve(train_losses, val_losses, save_path)



model2 = IntegratedModel(feature_extractor=model, clam_model=CLAM_SB()).to(device)
model2.load_state_dict(torch.load('crohn3.pt'))
...
plt.savefig('confusion_test3.png')
plt.close()
...
with open('results_test2__batch4.txt', 'w') as f:
    f.write(result_string_test)
    f.write('eproch50 batch 4')
