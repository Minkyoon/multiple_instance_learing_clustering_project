...
run = wandb.init(project='relapse', config={
    "user": "minkyoon",  # 여기에 사용자 이름을 적으세요.
...
print("Data Split:", config.data_split)
...
# val_dataset= CustomDataset(csv_file="valid.csv")
# test_dataset=CustomDataset(csv_file="test.csv")


# 데이터 로더 생성
dataset = CustomDataset(csv_file="/home/minkyoon/crohn/csv/label_data/relapse/relapse_label.csv")
...
torch.save(model_best.state_dict(), 'relapse.pt')
wandb.save('relapse.pt')
...
model2 = IntegratedModel(feature_extractor=model, clam_model=CLAM_SB()).to(device)
model2.load_state_dict(torch.load('relapse.pt'))
