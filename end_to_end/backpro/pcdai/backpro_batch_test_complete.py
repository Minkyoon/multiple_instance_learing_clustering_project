...
torch.save(model_best.state_dict(), 'resnet50_epoch100_trans3.pt')
...
model2 = IntegratedModel(feature_extractor=model, clam_model=CLAM_SB()).to(device)
model2.load_state_dict(torch.load('resnet50_epoch100_trans3.pt'))
...
#            total_loss_test += bag_weight * loss + (1-bag_weight) * instance_loss
            total_loss_test += bag_weight * loss 
...
with open('results_test3_이건subtypingtrue로하고clustering도함.txt', 'w') as f:
