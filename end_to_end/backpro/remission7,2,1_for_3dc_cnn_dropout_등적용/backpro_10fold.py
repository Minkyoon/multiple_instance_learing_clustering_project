...
class IntegratedModelWithCDCNN(nn.Module):
    def __init__(self, feature_extractor, clam_model, cdcnn_model):
        super(IntegratedModelWithCDCNN, self).__init__()
    ...
    def forward(self, x, label=None):
        ...
        # Combine CLAM and CDCNN outputs
...
for fold_num in range(10):
    ...
    optimizer = torch.optim.Adam(model2.parameters(), lr=0.00001, weight_decay=0.01 )
    bag_weight = 0.7
...
mean_values = {
    ...
    'ROC Score': df_results['ROC Score'].mean()
}

# Append the mean values to the DataFrame
df_results.loc[len(df_results)] = mean_values
