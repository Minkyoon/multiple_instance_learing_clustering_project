...
model = CLAM_SB()
state_dict = torch.load('/home/minkyoon/first/CLAM/results/remission_stratified_for_sysam_recommend_setting_7,2,1/task_1_tumor_vs_normal_CLAM_50_s1/s_3_checkpoint.pt')
...
max_score, max_index = torch.max(A_raw, dim=1)

# 가장 낮은 attention score를 가진 instance를 찾습니다.
...
# max_index와 min_index를 numpy 배열로 변환
max_index=max_index.item()
min_index=min_index.item() 

# max_index에 대한 이미지 표시
npy_path = data.iloc[max_index]['filepath']
image = np.load(npy_path)
plt.imshow(image, cmap='gray')
plt.title('Image for max_index')
plt.show()

# min_index에 대한 이미지 표시
npy_path = data.iloc[min_index]['filepath']
image = np.load(npy_path)
plt.imshow(image, cmap='gray')
plt.title('Image for min_index')
plt.show()
...
# attention score의 인덱스를 오름차순으로 정렬한 결과를 numpy 배열로 변환합니다.
sorted_indices = np.argsort(A_raw.cpu().numpy(), axis=1)

# 중간 인덱스 4개를 구합니다.
num_data = sorted_indices.shape[1]
mid_low_index = num_data // 2 - 2  # 중간값보다 작은 쪽에서 두 번째 인덱스
mid_high_index = num_data // 2 + 2  # 중간값보다 큰 쪽에서 두 번째 인덱스

# 중간 인덱스 4개에 해당하는 이미지들을 가져옵니다.
mid_indices = sorted_indices[0, mid_low_index:mid_high_index]
